...
mkcache_bytes(PyObject *self, PyObject *args) {
    ...
    vapash_light_t L = vapash_light_new(block_number);
...
calc_dataset_bytes(PyObject *self, PyObject *args) {
    ...
    if (cache_size % VAPASH_HASH_BYTES != 0) {
        char error_message[1024];
        sprintf(error_message, "The size of the cache must be a multiple of %i bytes (was %i)", VAPASH_HASH_BYTES, cache_size);
        PyErr_SetString(PyExc_ValueError, error_message);
        return 0;
    }
    ...
    vapash_compute_full_data(mem, &params, &cache);
...
hashimoto_light(PyObject *self, PyObject *args) {
    ...
    struct vapash_light *s;
    s = calloc(sizeof(*s), 1);
    s->cache = cache_bytes;
    s->cache_size = cache_size;
    s->block_number = block_number;
    struct vapash_h256 *h;
    h = calloc(sizeof(*h), 1);
    for (int i = 0; i < 32; i++) h->b[i] = header[i];
    struct vapash_return_value out = vapash_light_compute(s, *h, nonce);
...
hashimoto_full(PyObject *self, PyObject *args) {
    ...
    vapash_return_value out;
    vapash_params params;
    params.full_size = (size_t) full_size;
    vapash_full(&out, (void *) full_bytes, &params, (vapash_h256_t *) header, nonce);
    return Py_BuildValue("{" PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT ", " PY_CONST_STRING_FORMAT ":" PY_STRING_FORMAT "}",
                         "mix digest", &out.mix_hash, 32,
                         "result", &out.result, 32);
}
...
mine(PyObject *self, PyObject *args) {
    ...
    do {
        vapash_full(&out, (void *) full_bytes, &params, (const vapash_h256_t *) header, nonce++);
        // TODO: disagrees with the spec https://github.com/vaporyco/wiki/wiki/Vapash#mining
    } while (!vapash_check_difficulty(&out.result, (const vapash_h256_t *) difficulty));
...
get_seedhash(PyObject *self, PyObject *args) {
    ...
    if (!PyArg_ParseTuple(args, "k", &block_number))
        return 0;
    if (block_number >= VAPASH_EPOCH_LENGTH * 2048) {
        char error_message[1024];
        sprintf(error_message, "Block number must be less than %i (was %lu)", VAPASH_EPOCH_LENGTH * 2048, block_number);
...
static PyMethodDef PyvapashMethods[] =
        {
...
#if PY_MAJOR_VERSION >= 3
static struct PyModuleDef PyvapashModule = {
...
PyMODINIT_FUNC PyInit_pyvapash(void) {
    PyObject *module =  PyModule_Create(&PyvapashModule);
    // Following Spec: https://github.com/vaporyco/wiki/wiki/Vapash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) VAPASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) VAPASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) VAPASH_DATASET_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "CACHE_BYTES_INIT", (long) VAPASH_CACHE_BYTES_INIT);
    PyModule_AddIntConstant(module, "CACHE_BYTES_GROWTH", (long) VAPASH_CACHE_BYTES_GROWTH);
    PyModule_AddIntConstant(module, "EPOCH_LENGTH", (long) VAPASH_EPOCH_LENGTH);
    PyModule_AddIntConstant(module, "MIX_BYTES", (long) VAPASH_MIX_BYTES);
    PyModule_AddIntConstant(module, "HASH_BYTES", (long) VAPASH_HASH_BYTES);
    PyModule_AddIntConstant(module, "DATASET_PARENTS", (long) VAPASH_DATASET_PARENTS);
    PyModule_AddIntConstant(module, "CACHE_ROUNDS", (long) VAPASH_CACHE_ROUNDS);
    PyModule_AddIntConstant(module, "ACCESSES", (long) VAPASH_ACCESSES);
    return module;
}
#else
PyMODINIT_FUNC
initpyvapash(void) {
    PyObject *module = Py_InitModule("pyvapash", PyvapashMethods);
    // Following Spec: https://github.com/vaporyco/wiki/wiki/Vapash#definitions
    PyModule_AddIntConstant(module, "REVISION", (long) VAPASH_REVISION);
    PyModule_AddIntConstant(module, "DATASET_BYTES_INIT", (long) VAPASH_DATASET_BYTES_INIT);
    PyModule_AddIntConstant(module, "DATASET_BYTES_GROWTH", (long) VAPASH_DATASET_BYTES_GROWTH);
