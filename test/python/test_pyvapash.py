...
def test_get_cache_size_not_None():
    for _ in range(100):
        ...
        out = pyvapash.get_cache_size(block_num)
        assert out != None

def test_get_full_size_not_None():
    for _ in range(100):
        block_num = randint(0,12456789)
        out = pyvapash.get_full_size(block_num)
        assert out != None

def test_get_cache_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pyvapash.get_cache_size(block_num)
        out2 = pyvapash.get_cache_size((block_num // pyvapash.EPOCH_LENGTH) * pyvapash.EPOCH_LENGTH)
        assert out1 == out2

def test_get_full_size_based_on_EPOCH():
    for _ in range(100):
        block_num = randint(0,12456789)
        out1 = pyvapash.get_full_size(block_num)
        out2 = pyvapash.get_full_size((block_num // pyvapash.EPOCH_LENGTH) * pyvapash.EPOCH_LENGTH)
        assert out1 == out2

# See light_and_full_client_checks in test.cpp
def test_mkcache_is_as_expected():
    actual = pyvapash.mkcache_bytes(
        1024,
        "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~").encode('hex')
...
def test_calc_dataset_is_not_None():
    cache = pyvapash.mkcache_bytes(
              1024,
              "~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
    assert pyvapash.calc_dataset_bytes(1024 * 32, cache) != None
...
def test_light_and_full_agree():
    ...
    full_size = 1024 * 32
    header = "~~~~~X~~~~~~~~~~~~~~~~~~~~~~~~~~"
    light_result = pyvapash.hashimoto_light(full_size, cache, header, 0)
    dataset = pyvapash.calc_dataset_bytes(full_size, cache)
    full_result = pyvapash.hashimoto_full(dataset, header, 0)
...
def test_mining_basic():
    ...
    assert type(pyvapash.mine(dataset,header,easy_difficulty)) == dict
    assert type(pyvapash.mine(dataset,header,easy_difficulty)["nonce"]) == long
    assert type(pyvapash.mine(dataset,header,easy_difficulty)["mix digest"]) == str
    assert type(pyvapash.mine(dataset,header,easy_difficulty)["result"]) == str
...
def test_mining_doesnt_always_return_the_same_value():
    ...
    dataset = pyvapash.calc_dataset_bytes(full_size, cache)
    # Check type of outputs
    assert pyvapash.mine(dataset, header, easy_difficulty1)['nonce'] != pyvapash.mine(dataset, header, easy_difficulty2)['nonce']

def test_get_seedhash():
    assert pyvapash.get_seedhash(0).encode('hex') == '0' * 64
    import hashlib, sha3
    expected = pyvapash.get_seedhash(0)
    #print "checking seed hashes:",
    for i in range(0, 30000*2048, 30000):
        #print i // 30000,
        assert pyvapash.get_seedhash(i) == expected
